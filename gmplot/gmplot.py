...
class _Route(object):
    ...
    def __init__(self, origin, destination, **kwargs):
        ...
        self._strokeColor = _get_value(kwargs, ['strokeColor'], 'blue')
        self._strokeWeight = _get_value(kwargs, ['strokeWeight'], 5)
    ...
    def write(self, w):
        ...
        w.write('''  
                ...
                if (status == google.maps.DirectionsStatus.OK) {
                    new google.maps.DirectionsRenderer({map: map, suppressMarkers: true, polylineOptions: {strokeColor: '%s', strokeWeight: %d, strokeOpacity: 0.8}, traffic_model: 'optimistic'}).setDirections(response);
                }
            });
        ''' % (self._strokeColor, self._strokeWeight))
        w.write()
...
class _Marker(object):
    ...
    def write(self, w):
        ...
        if self._icon is not None: w.write('icon: %s,' % markerIcon)
...
class GoogleMapPlotter(object):
    ...
    def __init__(self, lat, lng, zoom, map_type='', apikey='', **kwargs):
        ...
        self.coloricon = os.path.join(os.path.dirname(__file__),
                                      'markers/%s.png')  # TODO: Unused; remove (counts as an API change).
    ...
    def grid(self, lat_start, lat_end, lat_increment, lng_start, lng_end,
             lng_increment):  # TODO: Replace start/end params with bounds parameter used elsewhere (counts as an API change).
        '''
        Plot a grid.
    ...
    def marker(self, lat, lng, color='#FF0000', c=None, title=None, precision=6, label=None, **kwargs):
        ...
        self.points.append((lat, lng, c or color, title, precision, label, kwargs.get('info_window'),
                            kwargs.get('draggable'), kwargs.get('IconURL')))
    ...
    def scatter(self, lats, lngs, color=None, size=None, marker=True, c=None, s=None, symbol='o', **kwargs):
        ...
        for i, location in enumerate(zip(lats, lngs)):
            ...
            if point_settings.pop('marker'):
                ...
                self._add_symbol(Symbol(point_settings.pop('symbol'), *location, size=point_settings.pop('size')),
                                 **point_settings)
    ...
    def heatmap(self, lats, lngs, threshold=None, radius=10, gradient=None, opacity=0.6, max_intensity=1,
                dissipating=True, precision=6, weights=None):
        '''
        Plot a heatmap.
    ...
    def write_points(self, w, color_cache=set()):
        ...
        for point in self.points:
            self.write_point(w, point[0], point[1], point[2], point[3], point[4], color_cache, point[5], point[6],
                             point[7], point[8])  # TODO: Not maintainable.

    def write_circles(self,
                      w):  # TODO: Remove since unused (counts as an API change since it's technically a public function). # pragma: no coverage
    ...
    def write_point(self, w, lat, lng, color, title, precision, color_cache, label, info_window=None, draggable=None,
                    IconURL='https://i.postimg.cc/k5gPZNQS/location-pin.png'):  # TODO: Bundle args into some Point or Marker class (counts as an API change).
        # Write the marker icon (if it isn't written already).
        ...
        marker = _Marker(_format_LatLng(lat, lng, precision), name=marker_name, title=title, label=label,
                         icon=marker_icon.name, draggable=draggable, IconURL=IconURL)
        marker.write(w)
    ...
    def write_circle(self, w, lat, long, size,
                     settings):  # TODO: Remove since unused (counts as an API change since it's technically a public function). # pragma: no coverage
